...
const data = `Tarih	Alt Kategori	Kategori	Dönem	Taksit
...
const csvContent = lines.map(line => {
    // Split by tab, but handle rows that might have different separation or typos
    let cols = line.split('\\t');
    if (cols.length < 2) cols = line.split(/ {2,}/); // Fallback to multiple spaces

    // Sanitize 20206 -> 2026 and 20225 -> 2025 typos
    return cols.map(c => c.trim().replace('20206', '2026').replace('20225', '2025')).join(',');
}).join('\\n');

fs.writeFileSync(path.join('database', 'cash_book_export.csv'), '\\ufeff' + csvContent);
console.log('CSV creation finished.');
