...
const data = `9 Eyl 2025	Kurum	Atölye	40	0532 256 93 27		 YETİŞKİN ATÖLYELERİ SUNUYOR. HAZIRLANINCA HABER VERİLECEK.
...
lines.forEach((line, i) => {
    ...
    if (parts.length < 7) {
        // Fallback for cases where tabs might have been converted to spaces
    ...
    const tipi = parts[1].trim();
    const branch = parts[2].trim();
    const ageStr = parts[3].strip ? parts[3].trim() : parts[3]; // Just in case
    const phone = parts[4].trim();
    const iletisim = parts[5].trim();
    let noteContent = parts[6].trim();

    let displayName = iletisim || "İsimsiz Aday";
    if (tipi === "Öğretmen") {
        displayName = `[Öğretmen] ${displayName}`;
    } else if (tipi === "Kurum") {
        displayName = f = `[Kurum] ${displayName}`;
    }

    let age = 0;
    const ageVal = parts[3].trim();
    if (ageVal) {
        if (ageVal.toUpperCase().includes("AY")) {
            const matches = ageVal.match(/\d+/);
            if (matches) {
                age = Math.round((parseFloat(matches[0]) / 12) * 10) / 10;
            }
        } else {
            age = parseFloat(ageVal.replace(',', '.')) || 0;
        }
    }
    ...
    rows.push({
        ...
        age: age,
        branch: branch || "Diğer",
        parent_name: iletisim || (displayName.startsWith('[') ? displayName.split('] ')[1] : displayName),
        phone: phone,
        source: source,
        status: 'Takip',
...
const csvHeader = 'student_name,age,branch,parent_name,phone,source,status,notes,created_at';
...
fs.writeFileSync('c:/Users/kaanc/AG_2026MyPNR/2026MyPNR/database/new_leads_import.csv', csvContent, 'utf8');
