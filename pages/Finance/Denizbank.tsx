...
const DENIZBANK_OPENING_BALANCES: Record<number, number> = {
    2025: 4087.20
};
...
const Denizbank: React.FC<DenizbankProps> = ({ canEdit = true }) => {
    ...
    const calculateOpeningBalanceForYear = (year: number, previousRecords: Pick<BankRecord, 'amount' | 'type' | 'date' | 'installment_info' | 'notes'>[]) => {
        const baseYear = Object.keys(DENIZBANK_OPENING_BALANCES)
            .map(Number)
            .filter(balanceYear => balanceYear <= year)
            .sort((a, b) => b - a)[0];

        const baseBalance = baseYear ? DENIZBANK_OPENING_BALANCES[baseYear] : 0;
        const fromDate = baseYear ? `${baseYear}-01-01` : '';
        const today = new Date().toISOString().split('T')[0];

        return previousRecords.reduce((acc, curr) => {
            if (fromDate && curr.date < fromDate) return acc;
            if (isPersonSplitChild(curr)) return acc;
            if (curr.installment_info && curr.date > today) return acc;
    ...
    const fetchData = async () => {
        ...
        try {
            ...
            setOpeningBalance(calculateOpeningBalanceForYear(year, allPrevRecords || []));
                    ...
                    data.forEach((row, idx) => {
                        ...
                        if (amountFound && amount !== 0) {
                            ...
                            const matchedRule = automationRules.find(rule => {
                                ...
                                return (ruleScopes.includes('all') || ruleScopes.includes('denizbank'))
    ...
    const confirmImport = async () => {
        ...
        try {
            ...
            for (const group of splitGroups) {
                ...
                const { error: childError } = await supabase.from('denizbank_book').insert(childRows);
                            ...
                            <table className="w-full text-left border-collapse text-xs">
                                <thead className="bg-slate-100 dark:bg-slate-900 text-slate-500 uppercase sticky top-0 z-10 shadow-sm">
                                    <tr>
                                        ...
                                        <th className="p-3 w-[32rem] min-w-[32rem]">Açıklama</th>
                                            ...
                                            return (
                                                ...
                                                <tr className={`hover:bg-slate-50 dark:hover:bg-slate-800/30 ${row.isDuplicate ? 'bg-amber-50 dark:bg-amber-900/10' : ''} ${!row.isSelected ? 'opacity-50 grayscale' : ''}`}>
                                                    ...
                                                    <td className="finance-description-cell p-2 align-middle" data-tooltip={row.description}>
                                                        <input
                                                            type="text"
                                                            className="w-[32rem] min-w-[32rem] bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded p-1.5 text-slate-700 dark:text-slate-200"
