...
const Vakifbank: React.FC<VakifbankProps> = ({ canEdit = true }) => {
    ...
    const normalizeSearchText = (value: any) => String(value ?? '')
        ...
        .normalize('NFD')
        .replace(/[\u0300-\u036f]/g, '');
    const normalizeStudentSearchValue = (value: any) => normalizeDottedIForCompare(value).toLocaleLowerCase('tr-TR');
    ...
    const calculateClosingBalance = (sourceRecords: BankRecord[]) => {
        ...
        const recordsToCalculate = latestDevir
            ? settledSourceRecords.filter(record => record.id === latestDevir.id || new Date(record.date).getTime() >= new Date(latestDevir.date).getTime())
            : settledSourceRecords;
    ...
    const normalizeExcelHeader = (value: any) => String(value ?? '')
        .replace(/\u00A0/g, ' ')
        .trim()
        .replace(/\s+/g, ' ')
        .toLocaleLowerCase('tr-TR')
                ...
                try {
                    ...
                    for (let i = 0; i < Math.min(data.length, 30); i++) {
                        ...
                        row.forEach((cell: any, idx: number) => {
                            ...
                            if (normalized.includes('aciklama') || normalized.includes('islem aciklamasi') || normalized.includes('islem tanimi')) descIdx = idx;
                            if (normalized === 'tip' || normalized === 'tur') typeIdx = idx;
                            if (normalized === 'kategori') categoryIdx = idx;
                            if (normalized.includes('alt kategori')) subCategoryIdx = idx;
                            if (normalized === 'donem') periodIdx = idx;
                            if (normalized.includes('taksit')) installmentIdx = idx;

                            const isBakiye = normalized.includes('bakiye') || normalized.includes('guncel');

                            if ((normalized.includes('tutar') || normalized.includes('borc') || normalized.includes('alacak') || normalized.includes('net')) && !isBakiye) {
                    ...
                    data.forEach((row, idx) => {
                        ...
                        if (amountFound && amount !== 0) {
                            ...
                            const matchedRule = automationRules.find(rule => {
                                ...
                                return (ruleScopes.includes('all') || ruleScopes.includes('vakifbank'))
    ...
    const confirmImport = async () => {
        ...
        try {
            ...
            for (const group of splitGroups) {
                ...
                const { error: childError } = await supabase.from('vakifbank_book').insert(childRows);
            ...
            setImportedRows([]);
            fetchData();
            alert(`${insertedCount} adet kayıt (taksitler ve kişi bölmeleri dahil) başarıyla oluşturuldu.`);
        ...
        try {
            ...
            if (formData.personSplitEnabled) {
                ...
                if (extraRows.length > 0) {
                    const { error: insertError } = await supabase.from('vakifbank_book').insert(extraRows);
                    if (insertError) throw insertError;
                }
            } else {
                const { error } = await supabase.from('vakifbank_book').insert(buildSplitRows());
    ...
    const normalizedStudentBranchSearch = normalizeStudentSearchValue(studentBranchSearch);
    const matchedCrmStudents = normalizedStudentBranchSearch
        ? crmStudents.filter(student => normalizeStudentSearchValue(student.name).includes(normalizedStudentBranchSearch))
        : [];
