...
import {
    ...
    FileText, Landmark, RefreshCcw, EyeOff, FileText as FileIcon, X, Download, Clock, Tag, Pencil, Calendar
} from 'lucide-react';
...
const DenizbankPOS: React.FC<DenizbankPOSProps> = ({ canEdit = true }) => {
    ...
    const fetchData = async () => {
        ...
        try {
            ...
            // 3. Fetch Selected Year Transactions from POS Book
    ...
    const handleManualSave = async (e: React.FormEvent) => {
        ...
        try {
            ...
            const deletePersonSplitChildren = async (parentRecordId: string) => {
                ...
                const { error } = await supabase.from('denizbank_pos_book').delete().in('id', childIds);
                if (error) throw error;
            };
            ...
            if (formData.personSplitEnabled) {
                ...
                if (editingId) {
                    ...
                    const { error: insertError } = await supabase.from('denizbank_pos_book').insert(buildPersonSplitChildRows(editingId));
                    ...
                    const { error: insertError } = await supabase.from('denizbank_pos_book').insert(buildPersonSplitChildRows(insertedParent.id));
                    if (insertError) throw insertError;
                }
            } else if (editingId) {
                await deletePersonSplitChildren(editingId);
                ...
                if (extraRows.length > 0) {
                    const { error: insertError } = await supabase.from('denizbank_pos_book').insert(extraRows);
                    if (insertError) throw insertError;
                }
            } else {
                const { error } = await supabase.from('denizbank_pos_book').insert(buildSplitRows());
    ...
    const handleDelete = async (id: string) => {
        ...
        try {
            ...
            const { error } = await supabase.from('denizbank_pos_book').delete().in('id', idsToDelete);
            if (error) throw error;
    ...
    const handleBulkDelete = async () => {
        ...
        try {
            ...
            const { error } = await supabase.from('denizbank_pos_book').delete().in('id', ids);
            if (error) throw error;
    ...
    const handleDownloadExcel = () => {
        ...
        XLSX.utils.book_append_sheet(workbook, worksheet, 'Denizbank POS');
        XLSX.writeFile(workbook, `denizbank-pos-${new Date().toISOString().split('T')[0]}.xlsx`);
    };
            ...
            <div className="flex flex-col md:flex-row items-center justify-between gap-4 bg-white dark:bg-pnr-card p-4 rounded-2xl border border-slate-200 dark:border-slate-800 shadow-sm">
                ...
                <div className="flex items-center gap-4">
                    ...
                    <div className="text-center min-w-24">
                        ...
                        <p className="text-xs text-slate-500 dark:text-slate-400 font-medium">Denizbank POS</p>
                    </div>
            ...
            {isManualModalOpen && (
                <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm">
                    <div className="bg-white dark:bg-pnr-card w-full max-w-2xl max-h-[90vh] overflow-y-auto rounded-2xl shadow-2xl border border-slate-200 dark:border-slate-700 animate-in zoom-in-95">
                        <div className="p-5 border-b border-slate-200 dark:border-slate-800 flex justify-between items-center bg-slate-50 dark:bg-slate-800/50">
                            <h3 className="font-bold text-lg text-slate-900 dark:text-white flex items-center gap-2">
                                <CreditCard size={20} className="text-pnr-purple" /> {editingId ? 'Denizbank POS Kaydını Düzenle' : 'Yeni Denizbank POS Kaydı'}
                            </h3>
