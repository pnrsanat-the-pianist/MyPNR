...
import { Users, UserPlus, RefreshCcw, Search, MessageSquare, CheckCircle2, XCircle, Clock, AlertCircle, CheckSquare, ArrowRight, TrendingDown, Calendar, TrendingUp, DollarSign, Wallet } from 'lucide-react';
...
import { BranchStat, Student } from '../types';
...
const Dashboard: React.FC = () => {
    ...
    // Stats
    const [leadStats, setLeadStats] = useState({ takip: 0, deneme: 0, kayit: 0, gorusuldu: 0 });
    ...
    const fetchDashboardData = async () => {
        ...
        `)
            ...
            if (leadsData) {
                console.log("Leads Data Statuses:", leadsData.map(l => l.status));
                const stats = { takip: 0, deneme: 0, kayit: 0, gorusuldu: 0 };
                leadsData.forEach((l: any) => {
                    const s = (l.status || '').toString().trim();
                    if (s === 'Takip') stats.takip++;
                    else if (s === 'Deneme') stats.deneme++;
                    else if (s === 'Kayıt') stats.kayit++;
                    // Extremely robust check for "Görüşüldü" (handles various encodings and typos)
    ...
    return (
        <div className="p-4 md:p-6 w-full max-w-full mx-auto min-h-screen">
            {/* Tablet scaling wrapper: visually shrinks by 15% (scale-85) on md screen range and expands width to compensate */}
            <div className="md:max-lg:scale-[0.85] md:max-lg:origin-top-left md:max-lg:w-[117.6%] space-y-6">
                ...
                <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 items-stretch">
                    ...
                    {/* LEFT: BRANCH DISTRIBUTION CHART (Span 4) - Full Height */}
                    <div className="lg:col-span-4 bg-white dark:bg-pnr-card border border-slate-200 dark:border-slate-800 rounded-3xl p-6 relative overflow-hidden shadow-sm dark:shadow-none flex flex-col min-h-[500px]">
                            ...
                            {loading ? (
                                ...
                                <div className="w-full h-full flex flex-col justify-center">
                                    ...
                                    <div className="text-center text-xs text-slate-400 mt-4 px-4">
                                        En yoğun branş <span style={{ color: chartData[0].color }} className="font-bold">{chartData[0].name}</span>, toplam öğrencinin %{((chartData[0].count / chartData.reduce((a, b) => a + b.count, 0)) * 100).toFixed(0)}'ini oluşturuyor.
                                    </div>
                                </div>
                            ) : (
