...
import { 
  ...
  ChevronDown, ChevronRight, Clock, ShieldCheck, Lock,
  X, History, Percent, MessageSquare, CalendarPlus,
  Unlock, ArrowRight, Layers, Music, Calendar, Plus, Save, Trash2, GraduationCap
} from 'lucide-react';
...
interface AttendanceRecord {
  id: string;
  lessonNumber: number; // Renamed from week for clarity
  date: string;
  time: string;
...
const Attendance: React.FC<AttendanceProps> = ({ currentUserRole }) => {
  ...
  const fetchData = async () => {
    ...
    try {
        ...
        const { data, error } = await supabase
            ...
            `)
            .eq('status', 'active') // Only fetch active periods
  ...
  // --- GROUPING LOGIC (UPDATED: Group by Teacher) ---
  ...
  const saveMakeup = async () => {
    if (makeupModal && newMakeup.date && newMakeup.time) {
  ...
  const handleOpenScheduleModal = (period: StudentPeriod) => {
      ...
      // Initialize form with existing or default
      if (period.scheduleConfig && period.scheduleConfig.length > 0) {
          setScheduleForm({
              count: period.weeklyLessonCount || 1,
              slots: period.scheduleConfig
          });
      } else {
          setScheduleForm({
              count: 1,
              slots: [{ dayIndex: 1, time: '10:00' }]
          });
      }
  };
  ...
  const updateSlotCount = (count: number) => {
      ...
      if (count > currentSlots.length) {
          for(let i = currentSlots.length; i < count; i++) {
              currentSlots.push({ dayIndex: 1, time: '10:00' });
          }
      } else {
  ...
  const calculateLessons = (startDateStr: string, slots: ScheduleSlot[], count = 10, startLessonNumber = 1) => {
      ...
      // Ensure slots are sorted by day index to find the "next" logical lesson
      const sortedSlots = [...slots].sort((a, b) => a.dayIndex - b.dayIndex);

      // Loop until we have 'count' lessons
      while(lessonsFound < count && loopSafety < 365) {
          ...
          for (const slot of matchingSlots) {
             ...
             generated.push({
                 ...
                 time: slot.time,
                 lessonNo: startLessonNumber + lessonsFound
             });
             lessonsFound++;
          }
  ...
  const saveSchedule = async () => {
      ...
      // Calculate start lesson number based on period number (Manual Schedule Reset)
      // Default rule: (PeriodNumber - 1) * 10 + 1. E.g. Period 2 starts at 11.
      ...
      try {
          ...
          // 2. Delete Existing Pending Lessons (Clean slate for replan)
  ...
  // --- NEW PERIOD & APPROVAL COMBINED LOGIC (DB) ---
  const handleApproveAndRollover = async (periodId: string) => {
      ...
      try {
          ...
          // 2. Calculate next start date
          const lastRecord = period.records[period.records.length - 1]; // Assume last record is end date
          ...
          // 3. Determine start lesson number for new period
          // Find max lesson number in current period and add 1
          const maxLessonNo = period.records.length > 0 
            ? Math.max(...period.records.map(r => r.lessonNumber)) 
            : (period.periodNumber * 10);
          
          const startNextLessonNo = maxLessonNo + 1;

          // Use existing schedule config
          const scheduleToUse = period.scheduleConfig.length > 0 
            ? period.scheduleConfig 
            : [{dayIndex: 1, time: '10:00'}]; // Fallback
          ...
          // 5. Generate Lessons for New Period (CONTINUING NUMBERING)
          const newLessons = calculateLessons(nextPeriodSearchStartStr, scheduleToUse, 10, startNextLessonNo);
          ...
          await supabase.from('instrument_attendance').insert(recordsToInsert);
  ...
  const isPeriodFullyApproved = (records: AttendanceRecord[]) => {
      return records.length > 0 && records.every(r => r.isApproved === true);
  };
  ...
  return (
    <div className="p-4 md:p-6 max-w-[1600px] mx-auto space-y-6">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          <h1 className="text-2xl md:text-3xl font-bold text-slate-900 dark:text-white tracking-tight font-display">Yoklama (Enstrüman)</h1>
          <p className="text-sm md:text-base text-slate-500 dark:text-slate-400 mt-1">
            CRM kayıtlı aktif enstrüman öğrencileri ve ders takibi.
          </p>
        </div>
        <div className="relative w-full md:w-64">
           ...
           <input 
             ...
             className="w-full bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 text-slate-900 dark:text-white pl-10 pr-4 py-2 rounded-xl focus:outline-none focus:ring-2 focus:ring-pnr-purple"
           />
        </div>
      </div>
      ...
      {loading && (
          <div className="text-center py-10 text-slate-500">
              <div className="w-8 h-8 border-4 border-pnr-purple/30 border-t-pnr-purple rounded-full animate-spin mx-auto mb-2"></div>
              Veriler yükleniyor...
          </div>
      )}
      ...
      {!loading && Object.keys(groupedPeriods).length === 0 && (
          <div className="text-center py-12 bg-white dark:bg-pnr-card rounded-2xl border border-slate-200 dark:border-slate-800">
             ...
             <p className="text-slate-500 font-medium">Bu kriterlere uygun aktif enstrüman öğrencisi bulunamadı.</p>
          </div>
      )}

      {/* Grouped Lists (Grouped by Teacher) */}
            ...
            {(periods as StudentPeriod[]).map((period) => {
                ...
                return (
                    ...
                    <div 
                        ...
                        className={`
                        p-4 flex flex-col md:flex-row md:items-center gap-4 cursor-pointer transition-colors
                        ${isExpanded ? 'bg-slate-50 dark:bg-slate-800/50 rounded-t-2xl' : 'hover:bg-slate-50 dark:hover:bg-slate-800/30 rounded-2xl'}
                        `}
                    >
                        {/* User Info */}
                        <div className="flex items-center gap-4 flex-1">
                        <div className="w-10 h-10 rounded-full bg-gradient-to-br from-pnr-blue to-cyan-500 text-white flex items-center justify-center font-bold text-sm shrink-0 shadow-lg shadow-pnr-blue/20">
                            {period.studentName.substring(0,2).toUpperCase()}
                        </div>
                        <div>
                            <h3 className="font-bold text-slate-900 dark:text-white text-base">{period.studentName}</h3>
                        ...
                        {isManager && (
                            <button 
                                onClick={(e) => { e.stopPropagation(); handleOpenScheduleModal(period); }}
                                className="flex items-center gap-1 text-xs font-bold text-pnr-purple bg-purple-50 hover:bg-purple-100 px-3 py-1.5 rounded-lg border border-purple-200 transition-colors whitespace-nowrap"
                            >
                        ...
                        {/* Approval Badge */}
                        {isFullyApproved ? (
                            <div className="flex items-center gap-1 text-green-600 dark:text-green-500 bg-green-50 dark:bg-green-900/20 px-3 py-1 rounded-full text-xs font-bold border border-green-100 dark:border-green-800 animate-pulse">
                    ...
                    {isExpanded && (
                        ...
                        {hasNoRecords ? (
                            <div className="text-center py-8">
                                <p className="text-slate-500 mb-2">Henüz ders programı oluşturulmamış.</p>
                                <button 
                                    onClick={() => handleOpenScheduleModal(period)}
                                    className="bg-pnr-purple text-white px-4 py-2 rounded-lg font-bold text-sm shadow-lg hover:bg-pnr-indigo"
                                >
                                    Dersleri Oluştur
                                </button>
                            </div>
                            ...
                            <div className="grid grid-cols-2 md:grid-cols-5 lg:grid-cols-10 gap-3 pb-8 items-start"> 
                                {sortedRecords.map((record, index) => {
                                    ...
                                    const canEdit = !isRecordLocked && (isManager || !record.isApproved); 

                                    return (
                                    <div key={record.id} className="relative group flex flex-col gap-2">
                                        <div 
                                        onClick={() => canEdit && setSelectedCell(selectedCell?.recordId === record.id ? null : { periodId: period.id, recordId: record.id })}
                                        className={`
                                            flex flex-col items-center justify-center rounded-xl border transition-all min-h-[10rem] relative overflow-hidden
                                            ${config.color}
                                            ${!canEdit ? 'opacity-90 cursor-not-allowed ring-1 ring-slate-200 dark:ring-slate-700' : 'cursor-pointer hover:shadow-md hover:scale-[1.02]'}
                                        ...
                                        <div className="flex-1 flex flex-col items-center justify-center p-3 w-full">
                                            <div className="flex items-center gap-1 mb-1 h-4">
                                                {record.isMakeup ? (
                                                    <span className="text-xs font-bold text-pnr-purple uppercase tracking-wider">
                                                        Telafi
                                                    </span>
                                                ) : !hideLessonNumber ? (
                                                    <span className="text-[10px] font-bold opacity-60 uppercase">{record.lessonNumber}. Ders</span>
                                                ) : null}
                                            </div>
                                        ...
                                        {record.note && !record.isMakeup && (
                                            <div className="bg-slate-100 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-lg p-2 text-[10px] text-slate-600 dark:text-slate-300 leading-tight shadow-sm">
                                                <div className="flex items-center gap-1 mb-0.5 font-bold text-pnr-purple">
                                                    <MessageSquare size={10} /> Telafi:
                                                </div>
                                                {record.note}
                                            </div>
                                        )}
                                        ...
                                        {/* MAKEUP BUTTON */}
                                        {record.status === 'makeup_needed' && !record.isMakeup && (
                                            <div className="mt-1">
                                                {isManager && (
                                                    <button 
                                                        onClick={() => setMakeupModal({ periodId: period.id, sourceRecordId: record.id })}
                                                        className="w-full bg-white dark:bg-slate-800 border border-pnr-orange text-pnr-orange hover:bg-orange-50 dark:hover:bg-orange-900/20 text-[10px] font-bold py-1.5 rounded-lg transition-colors flex items-center justify-center gap-1 shadow-sm"
                                                    >
                                        ...
                                        <div className={`absolute left-0 w-48 bg-white dark:bg-slate-800 rounded-xl shadow-2xl border border-slate-200 dark:border-slate-700 p-1.5 flex flex-col gap-1 z-[60] animate-in zoom-in-95 ${isBottomRow ? 'bottom-full mb-2 origin-bottom-left' : 'top-full mt-2 origin-top-left'}`}>
                                            {(Object.keys(STATUS_CONFIG) as AttendanceStatus[])
                                                .filter(statusKey => statusKey !== 'pending')
                                                .map((statusKey) => (
                                            <button
                                                key={statusKey}
                                                onClick={() => handleStatusSelect(period.id, record.id, statusKey)}
                                                className={`
                                                flex items-center gap-2 px-2 py-2 rounded-lg text-xs font-medium text-left transition-colors
                                                ${record.status === statusKey ? 'bg-pnr-purple text-white' : 'hover:bg-slate-100 dark:hover:bg-slate-700 text-slate-700 dark:text-slate-300'}
                                                `}
                                            >
                        ...
                        <div className="flex justify-between items-center border-t border-slate-200 dark:border-slate-800 pt-4 flex-wrap gap-4">
                            <div className="text-xs text-slate-500">
                                {isFullyApproved ? (
                                    <span className="flex items-center gap-1 text-green-600"><CheckCircle2 size={14}/> Tüm Dersler Onaylandı</span>
                                ) : (
                                    <span className="flex items-center gap-1"><ShieldCheck size={14}/> Yeni dönem için tüm dersleri onaylayın</span>
                                )}
                            </div>
                            
                            <div className="flex gap-3">
                                {isFullyApproved && isManager && (
                                    <button 
                                        onClick={() => handleApproveAndRollover(period.id)}
                                        className="bg-pnr-purple hover:bg-pnr-indigo text-white px-4 py-2 rounded-xl font-bold text-sm shadow-lg shadow-pnr-purple/20 flex items-center gap-2 transition-colors"
                                    >
                                        <CheckCircle2 size={16} />
                                        Periyodu Onayla & Yeni Dönem Aç
                                    </button>
                                )}
                            </div>
                        </div>
          ...
          <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm">
             <div className="bg-white dark:bg-pnr-card w-full max-w-lg rounded-2xl shadow-2xl flex flex-col overflow-hidden border border-slate-200 dark:border-slate-700 animate-in zoom-in-95">
                 <div className="p-5 border-b border-slate-200 dark:border-slate-800 bg-slate-50 dark:bg-slate-800/50 flex justify-between items-center">
                    <div>
                        <h3 className="text-lg font-bold text-slate-900 dark:text-white flex items-center gap-2">
                           <Calendar className="text-pnr-purple" size={20} />
                           Yeni Periyot Planla
                        </h3>
                        <p className="text-xs text-slate-500">10 derslik periyot oluşturulacak.</p>
                    </div>
                 ...
                 <div className="p-6 space-y-6">
                    ...
                    <div>
                        <label className="block text-sm font-bold text-slate-700 dark:text-slate-300 mb-2">Başlangıç Tarihi</label>
                        <input 
                            type="date"
                    ...
                    <div>
                        <label className="block text-sm font-bold text-slate-700 dark:text-slate-300 mb-2">Haftada Kaç Ders?</label>
                        <div className="flex gap-2">
                            {[1, 2].map(n => (
                                <button
                                    key={n}
                                    onClick={() => updateSlotCount(n)}
                                    className={`w-10 h-10 rounded-lg font-bold border ${scheduleForm.count === n ? 'bg-pnr-purple text-white border-pnr-purple' : 'bg-white dark:bg-slate-800 border-slate-200 dark:border-slate-700 text-slate-600 dark:text-slate-300'}`}
                                >
                                    {n}
                                </button>
                            ))}
                        ...
                        <p className="text-xs text-slate-500 mt-2 font-medium">
                            {scheduleForm.count === 1 ? '10 haftalık periyot (Toplam 10 ders)' : '5 haftalık periyot (Toplam 10 ders)'}
                        </p>
                    </div>
                    ...
                    <div className="space-y-3 max-h-[300px] overflow-y-auto pr-2 custom-scrollbar">
                        {scheduleForm.slots.map((slot, index) => (
                            <div key={index} className="p-3 bg-slate-50 dark:bg-slate-900/50 border border-slate-200 dark:border-slate-700 rounded-xl flex items-center gap-3">
                                <div className="w-6 h-6 rounded-full bg-slate-200 dark:bg-slate-700 flex items-center justify-center text-xs font-bold text-slate-600 dark:text-slate-400">
                                    {index + 1}
                                </div>
                                <div className="flex-1">
                                    <label className="text-[10px] font-bold text-slate-400 uppercase">Gün</label>
                                    <select 
                                        className="w-full bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-lg p-1.5 text-sm"
                                        value={slot.dayIndex}
                                        onChange={(e) => updateSlot(index, 'dayIndex', parseInt(e.target.value))}
                                    >
                                        {DAYS.map(d => <option key={d.value} value={d.value}>{d.label}</option>)}
                                    </select>
                                </div>
                                <div className="w-24">
                                    <label className="text-[10px] font-bold text-slate-400 uppercase">Saat</label>
                                    <input 
                                        type="time" 
                                        className="w-full bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-lg p-1.5 text-sm"
                                        value={slot.time}
                    ...
                    <div className="bg-blue-50 dark:bg-blue-900/10 p-3 rounded-lg flex gap-2 text-xs text-blue-700 dark:text-blue-300">
                        <AlertCircle size={16} className="shrink-0" />
                        <p>Bu işlem mevcut "Bekleyen" dersleri silip, seçtiğiniz programa göre 10 yeni ders tarihi oluşturacaktır. Geçmiş dersler etkilenmez.</p>
                    </div>
        ...
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm">
            <div className="bg-white dark:bg-pnr-card w-full max-w-sm rounded-2xl shadow-2xl p-6 border border-slate-200 dark:border-slate-700 animate-in zoom-in-95">
                <h3 className="text-lg font-bold text-slate-900 dark:text-white mb-2 flex items-center gap-2">
                   <AlertCircle className="text-pnr-orange" size={20} />
                   Telafi Nedeni
                </h3>
                <div className="space-y-3 mb-4 mt-4">
                    {MAKEUP_REASONS.map((reason) => (
                        <label key={reason} className="flex items-center gap-3 p-3 rounded-xl border border-slate-200 dark:border-slate-700 cursor-pointer hover:bg-slate-50 dark:hover:bg-slate-800/50 transition-colors">
                            <input 
                                type="radio"
                                name="makeupReason"
                                value={reason}
                                checked={noteModal.reason === reason}
                                onChange={(e) => setNoteModal({...noteModal, reason: e.target.value})}
                                className="w-4 h-4 text-pnr-orange focus:ring-pnr-orange"
                            />
                            <span className="text-sm text-slate-700 dark:text-slate-300 font-medium">{reason}</span>
                        </label>
                    ))}
                </div>
                {noteModal.reason === 'Özel Durum' && (
                    <div className="mb-4">
                        <textarea 
                            autoFocus
                            className="w-full bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-lg p-3 text-sm focus:ring-2 focus:ring-pnr-orange focus:outline-none"
                            rows={2}
                            placeholder="Detay..."
                            value={noteModal.customDetail}
                            onChange={(e) => setNoteModal({...noteModal, customDetail: e.target.value})}
                        ></textarea>
                    </div>
                )}
                <div className="flex justify-end gap-3">
                    <button onClick={() => setNoteModal(null)} className="text-slate-500 text-sm font-medium">İptal</button>
                    <button onClick={saveNote} className="bg-pnr-orange text-white px-4 py-2 rounded-lg text-sm font-bold">Kaydet</button>
                </div>
            </div>
        </div>
      )}
      ...
      {makeupModal && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm">
            <div className="bg-white dark:bg-pnr-card w-full max-w-sm rounded-2xl shadow-2xl p-6 border border-slate-200 dark:border-slate-700 animate-in zoom-in-95">
                <h3 className="text-lg font-bold text-slate-900 dark:text-white mb-2 flex items-center gap-2">
                    <CalendarPlus className="text-pnr-purple" size={20} />
                    Telafi Dersi
                </h3>
                <div className="space-y-4 mt-4">
                    <div>
                        <label className="block text-xs font-bold text-slate-500 uppercase mb-1">Tarih</label>
                        <input type="date" className="w-full bg-slate-50 dark:bg-slate-800 border rounded-lg p-2.5 text-sm" value={newMakeup.date} onChange={(e) => setNewMakeup({...newMakeup, date: e.target.value})} />
                    </div>
                    <div>
                        <label className="block text-xs font-bold text-slate-500 uppercase mb-1">Saat</label>
                        <input type="time" className="w-full bg-slate-50 dark:bg-slate-800 border rounded-lg p-2.5 text-sm" value={newMakeup.time} onChange={(e) => setNewMakeup({...newMakeup, time: e.target.value})} />
                    </div>
                </div>
                <div className="flex justify-end gap-3 mt-6">
                    <button onClick={() => setMakeupModal(null)} className="text-slate-500 text-sm font-medium">İptal</button>
                    <button onClick={saveMakeup} className="bg-pnr-purple text-white px-4 py-2 rounded-lg text-sm font-bold">Planla</button>
                </div>
            </div>
        </div>
      )}
